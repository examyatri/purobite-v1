...
// ✅ NETLIFY HANDLER - replaces `module.exports = async (req, res) => {}`
...
return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: ‘POST only’ }) };
}
...
const { action, data = {}, apiKey } = parsed;

```
if (apiKey !== SECURE_API_KEY) {
  return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: 'Unauthorized' }) };
}
...
switch (action) {
  ...
  default: return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: 'Unknown action: ' + action }) };
}
